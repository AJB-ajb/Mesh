-- Migration: Remove deprecated columns from matching RPC return types
-- Expand phase: remove `mode` from match_postings_to_user, remove
-- `availability_slots` from match_users_to_posting. The actual table columns
-- will be dropped in a follow-up contract migration after this code is deployed.

-- Drop existing overloads so we can redefine with new RETURNS TABLE
...
-- ============================================
-- match_postings_to_user() — remove `mode` from RETURNS TABLE
-- ============================================
...
COMMENT ON FUNCTION match_postings_to_user IS 'Finds top matching postings for a user. Returns visibility instead of deprecated mode column. Includes availability hard filter.';

-- ============================================
-- match_users_to_posting() — remove `availability_slots` from RETURNS TABLE
-- ============================================
...
COMMENT ON FUNCTION match_users_to_posting IS 'Finds top matching users for a posting. Removed deprecated availability_slots from return type. Includes availability hard filter.';
