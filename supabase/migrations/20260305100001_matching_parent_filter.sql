-- Update matching RPCs to support parent_posting_id filtering.
-- - match_postings_to_user: replace context_filter with parent_posting_id_filter
--   When set: only return child postings of the given parent
--   When NULL in discover mode: exclude child postings from global discover
-- - match_users_to_posting: add parent_posting_id_param to restrict candidate pool

-- Drop existing functions to redefine with new signatures
...
-- ============================================
-- match_postings_to_user() — updated
-- ============================================
...
CREATE OR REPLACE FUNCTION match_postings_to_user(
  ...
  parent_posting_id_filter uuid DEFAULT NULL
)
RETURNS TABLE (
  posting_id uuid,
  similarity float,
...
BEGIN
  ...
  WHERE
    ...
    -- Hard filter: parent posting scope
    -- When parent_posting_id_filter is set: only children of that parent
    -- When NULL: exclude child postings from global discover
    AND (
      (parent_posting_id_filter IS NOT NULL AND p.parent_posting_id = parent_posting_id_filter)
      OR (parent_posting_id_filter IS NULL AND p.parent_posting_id IS NULL)
    )
    -- Hard filter: category
    AND (category_filter IS NULL OR p.category = category_filter)
    -- Hard filter: context identifier (kept for backward compat)
...
COMMENT ON FUNCTION match_postings_to_user IS 'Finds top matching postings for a user. Includes hard filters for category, context, location, availability, skill overlap, and parent posting scope.';

-- ============================================
-- match_users_to_posting() — updated
-- ============================================
...
CREATE OR REPLACE FUNCTION match_users_to_posting(
  ...
  max_distance_km double precision DEFAULT NULL,
  parent_posting_id_param uuid DEFAULT NULL
)
RETURNS TABLE (
  user_id uuid,
  similarity float,
...
BEGIN
  ...
  WHERE
    ...
    -- When parent_posting_id_param is set, restrict to parent's team members
    AND (
      parent_posting_id_param IS NULL
      OR pr.user_id IN (SELECT unnest(get_posting_team_member_ids(parent_posting_id_param)))
    )
...
COMMENT ON FUNCTION match_users_to_posting IS 'Finds top matching users for a posting. Includes hard filters for location, availability, skill overlap, and parent posting scope.';
